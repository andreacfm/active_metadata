    ...
    module Base
      ...
      module InstanceMethods
        ...
        def create_note_for(field, note, created_by=nil)
          ...
          ActiveMetadata::Base::Config.notes.insert :note => note, :id => metadata_id, :field => field , :created_at => Time.now.utc, :created_by => created_by, :updated_at => Time.now.utc
        end

        def update_note id, note, updated_by=nil
          ActiveMetadata::Base::Config.notes.update({:_id => id}, {"$set" => {:note => note, :updated_at => Time.now.utc, :updated_by => updated_by}})
        end

        def notes_for field
          ActiveMetadata::Base::Config.notes.find({:id => metadata_id, :field => field},{:sort => [[:updated_at , 'descending']]}).to_a
        end
        ...
        def delete_note id
          ActiveMetadata::Base::Config.notes.remove({:_id => id})
        end

        # History
        def save_history
          self.changes.each do |key,value| next if HISTORY_SKIPS.include?(key)
            ActiveMetadata::Base::Config.history.insert :id => metadata_id, :field => key, :value => value[1], :created_at => Time.now.utc
          end
        end

        def history_for field
          ActiveMetadata::Base::Config.history.find({:id => metadata_id, :field => field}, {:sort => [[:created_at,'descending']]}).to_a
        end
